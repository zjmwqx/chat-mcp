...
class MCPToolCollector:
    ...
    async def collect_mcp_tools(self, enabled_mcps: Optional[List[MCPServer]]) -> List[MCPTool]:
        """
        从启用的MCP服务器收集工具列表
        ...
        logger.info(f"[MCP] Collecting tools from {len(enabled_mcps)} enabled MCP servers")
        ...
        for mcp_server in enabled_mcps:
            try:
                ...
                # 过滤被禁用的工具
                available_tools = self._filter_disabled_tools(tools, mcp_server.disabled_tools)
                
                logger.info(f"[MCP] Server {mcp_server.name}: {len(tools)} total tools, {len(available_tools)} available")
                
                # 添加到工具列表
                mcp_tools.extend(available_tools)
                
            except Exception as e:
                logger.error(f"[MCP] Failed to get tools from server {mcp_server.name}: {e}")
                continue
        
        logger.info(f"[MCP] Collected {len(mcp_tools)} total tools from enabled servers")
        return mcp_tools
    
    def _filter_disabled_tools(self, tools: List[MCPTool], disabled_tools: Optional[List[str]]) -> List[MCPTool]:
        """
        过滤被禁用的工具
        ...
        if filtered_count > 0:
            logger.info(f"[MCP] Filtered out {filtered_count} disabled tools: {disabled_tools}")
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            response.message.metadata.update({
                "enabled_servers": len(request.enabled_mcps) if request.enabled_mcps else 0,
                "collected_tools_count": len(mcp_tools),
                "step": 3,
                "stage": "ai_provider_integration_complete"
            })
            
            logger.info(f"[Chat] AI response generated successfully")
            logger.info(f"[Chat] Tool calls detected: {len(response.message.tool_calls) if response.message.tool_calls else 0}")
